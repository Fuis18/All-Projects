...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const in_color = document.querySelector(".text input");

    const color_run = () => {
      if (!network) return;  // Verificar si la red está entrenada
      let entrada = {
    ...
    in_color.addEventListener("change", () => {
    ...
    <Main title="IA Text & Background" className="cont__pages">
      <div className="f7">
        ...
        <div className="text">
          ...
          <input type="checkbox" value="color" required/>
        </div>
        <div id="sitio">Mi sitio Web</div>
        <Script
          src = "/pages/7/brain.js"
          strategy="lazyOnload"
          onload={() => {
            if (network == null) {
              ...
              setNetwork(network); // Actualizar el estado con la red entrenada
            }
            console.log("Red entrenada:", network);
          }}
        />
      </div>
    </Main>
